...
**TOOL AVAILABILITY:** direct prompting may be a deferred tool. If calling it fails, use direct prompting to fetch the schema first, then retry. NEVER skip setup because of tool issues.
...
1. Check script exists: `[ -f "scripts/validate-docs.cjs" ]`
2. If exists: run `node scripts/validate-docs.cjs docs/`
