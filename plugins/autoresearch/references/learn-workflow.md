...
**CRITICAL — BLOCKING PREREQUISITE:** If invoked without `--mode` or sufficient inline context, you MUST use `direct question` to gather config BEFORE proceeding to Phase 1.

**TOOL AVAILABILITY:** `direct question` may be a deferred tool. If calling it fails, use `repo search` to fetch the schema first, then retry. NEVER skip setup because of tool issues.
...
### Questions (single direct question call — 4 questions)
...
4. **Strategy by count:**
   - **0 files:** Warn via direct question: "No docs found. Switch to init mode? [yes/cancel]". If yes → restart as init. If cancel → STOP.
...
1. Check script exists: `[ -f "$HOME/.codex/scripts/validate-docs.cjs" ]`
2. If exists: run `node $HOME/.codex/scripts/validate-docs.cjs docs/`
