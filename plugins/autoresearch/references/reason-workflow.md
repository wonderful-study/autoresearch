...
**CRITICAL — BLOCKING PREREQUISITE:** If `/autoresearch:reason` is invoked without task, domain, and mode all provided, you MUST use `direct question` to gather context BEFORE proceeding to Phase 1. DO NOT skip this step.

**TOOL AVAILABILITY:** `direct question` may be a deferred tool. If calling it fails, use `repo search` to fetch the schema first, then retry.
...
Batch ALL selected questions into a SINGLE `direct question` call:
...
**STOP: Have you completed the Interactive Setup above?** Complete `direct question` before entering this phase.
