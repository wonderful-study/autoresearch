...
**CRITICAL — BLOCKING PREREQUISITE:** If `/autoresearch:predict` is invoked without scope, goal, and depth all provided, you MUST use `direct question` to gather context BEFORE proceeding to ANY phase. DO NOT skip this step. DO NOT jump to Phase 1 without completing interactive setup.

**TOOL AVAILABILITY:** `direct question` may be a deferred tool. If calling it fails or the schema is not available, you MUST use `repo search` to fetch the `direct question` schema first, then retry. NEVER skip interactive setup because of a tool fetch issue — resolve tool availability, then ask the questions.
...
**IMPORTANT:** Batch ALL selected questions into a SINGLE `direct question` call. NEVER ask one at a time — users need full context to make informed decisions together.
...
**STOP: Have you completed the Interactive Setup above?** If invoked without scope/goal/depth, you MUST complete the `direct question` call BEFORE entering this phase.
