...
**CRITICAL — BLOCKING PREREQUISITE:** If `/autoresearch:security` is invoked without `--diff`, scope, or focus, you MUST scan the codebase first, then use `direct question` to gather user input BEFORE proceeding to ANY phase. DO NOT skip this step.
