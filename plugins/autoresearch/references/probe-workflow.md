...
**CRITICAL — BLOCKING PREREQUISITE:** If `/autoresearch:probe` is invoked without a topic description, you MUST use `request_user_input` when available, or one concise batched direct prompt, to gather context BEFORE proceeding to ANY phase. DO NOT skip this step. DO NOT jump to Phase 1 without completing interactive setup.

**TOOL AVAILABILITY:** If structured input is unavailable in the current Codex runtime, ask the selected questions in one concise direct message. NEVER skip interactive setup because a structured-question helper is unavailable.
...
You MUST ask ALL selected questions in a SINGLE batched prompt:
...
**IMPORTANT:** Batch ALL selected questions into a SINGLE prompt. NEVER ask one at a time — users need full context to make informed decisions together. If structured input only supports one question per call, include all questions in a single direct message with numbered headers.
...
- If the user cancels any setup prompt → exit cleanly: "Probe cancelled. Run `/autoresearch:probe` again when ready." Output directory is NOT created — no partial files.
...
**STOP: Have you completed the Interactive Setup above?** Complete the batched setup prompt before entering this phase.
...
Issue a single batched prompt with the ≤5 synthesized questions. Each question is presented with its originating persona label so the user understands the interrogation angle.
...
**Autonomous mode (`--mode autonomous`):** Substitutes a self-answer step. Codex uses prior-art ledger + persona reasoning to produce best-effort answers, each marked with a confidence level:
